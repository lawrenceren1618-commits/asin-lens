...
## 2026-09-01 夜 — 异动归档空分支；美西 Cron 待 9/2 验

- **空分支**：上一笔 `effectiveTo` 早于业务日且指标有变化时，旧段保持、插入新 `asin_change_log` 再归档（`planChangeLogWrite`）。不再吞掉缺口后的异动。不回填 7/23 之后的历史行
- **单测**：`change-log.test.ts`
- **飞书群 webhook**：本轮不做
- **库基线（9/1 夜查）**：快照/双报告最新是上海日标的 `2026-09-01`；`asin_change_log` 仍各 ASIN 一行 `2026-07-23`
- **live 验（待 9/2）**：Vercel `0 10 * * *` UTC ≈ 美西 03:00 PDT ≈ 北京 18:00。业务日应是 `2026-09-01`，但该日快照已在，接口多半 `skipped: already_collected`（看 JSON 的 `reportDate`，不要指望新插一行）。11:00 UTC 同样 skip。此刻勿 `?force=1`（会写成 `2026-08-31`）。真正新业务日写入要等 **9/3 美西 03:00**（标 `2026-09-02`）。空分支修复须先 push/部署后，下一次**实际采集**才会插新异动行

...
import { listProjects, listProjectAsins } from "@/lib/db/queries";
...
export async function generateDailyReports(reportDate = shanghaiYesterday()) {
  ...
  for (const project of allProjects) {
    const projectAsins = await listProjectAsins(project.id);
    ...
    const email = await sendReportEmail({
      subject: `[asin-lens] ${project.name} ${reportDate} 日报`,
      text: markdown,
    });
